...
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def forward(self, features, proposals, targets=None):
        ...
        if not self.cfg.MODEL.TRANS_HEAD_ON:
            x_box, detections, loss_box = self.box(features, proposals, targets)
        else:
            x_box, detections_all, detections, loss_box = self.box(features, proposals, targets)
        ...
        if self.cfg.MODEL.MASK_ON:
            ...
            if self.mask.training and self.cfg.MODEL.ROI_MASK_HEAD.SHARE_BOX_FEATURE_EXTRACTOR:
                mask_features = x_box
        ...
        if self.cfg.MODEL.CAR_CLS_HEAD_ON:
            ...
            if self.car_cls_rot.training and self.cfg.MODEL.ROI_MASK_HEAD.SHARE_BOX_FEATURE_EXTRACTOR:
                car_cls_rot_features = x_box
            ...
            if self.cfg.MODEL.TRANS_HEAD_ON:
                detections, loss_trans = self.trans(x_car_cls_rot, detections_all, detections, targets)
                losses.update(loss_trans)

        return x_box, detections, losses
